...
func TestOneSpec(t *testing.T) {
	...
	expected := travellerBot{countryStep:5,numInstances:2,probs:newYearProbs(&(params.BotSpecs[0]))}
...
func TestTwoSpecs(t *testing.T) {
	...
	expected := travellerBot{countryStep:10,numInstances:1,probs:newYearProbs(&(params.BotSpecs[0]))}
	if !reflect.DeepEqual(ts.bots[0],expected) {
		t.Error("traveller bot has incorrect value",ts.bots[0],expected)
	}
...
func TestThreeSpecs(t *testing.T) {
	...
	expected := travellerBot{countryStep:60,numInstances:1,probs:newYearProbs(&(params.BotSpecs[0]))}
	if !reflect.DeepEqual(ts.bots[0],expected) {
		t.Error("traveller bot has incorrect value",ts.bots[0],expected)
	}
	expected = travellerBot{countryStep:30,numInstances:2,probs:newYearProbs(&(params.BotSpecs[1]))}
	if !reflect.DeepEqual(ts.bots[1],expected) {
		t.Error("traveller bot has incorrect value",ts.bots[1],expected)
	}
	expected = travellerBot{countryStep:7.5,numInstances:8,probs:newYearProbs(&(params.BotSpecs[2]))}
